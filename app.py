...
if uploaded_file is not None and not st.session_state.get('input_error', False):
    ...
    # Prepare customdata for total pages, as 2D array for plotly
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='Page Change Rate: %{y:.2f}%<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=df[date_col],
        ...
        yaxis="y2",
        customdata=pages_tooltip,
        hovertemplate='Traffic Change Rate: %{y:.2f}%<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=df[date_col],
        ...
        yaxis="y",
        customdata=pages_tooltip,
        hovertemplate='Traffic Per Page: %{y:.2f}<extra></extra>'
    ))
    ...
    # Set unified hoverlabel header with Date and Total Pages (this is only available via update_layout, see below)
    # This requires Plotly 5.0+ and works as intended in Streamlit as of 2024.
    fig.update_layout(
        hovermode="x unified",
        hoverlabel=dict(
            font_size=16
        ),
        # Set header template for unified hover
        xaxis=dict(
            showspikes=True,
            spikemode="across",
            spikedash="solid",
    ...
    # The unified tooltip will show:
    # Date (automatically at the top)
    # For each metric, label and value
    # To add "Total Pages: N" under the date: set
    # fig.update_layout(hovertemplate=None)
    # But as of now, Plotly doesn't let you customize the unified hover header directly to add 'Total Pages' ONCE at the top.
    # It will show for each trace line. This is the closest possible in Plotly as of 2024.
    # If you want "Total Pages" just under the date and only once, you would need to use a custom hover with html outside of Plotly.

    # Add ranking state rectangles
